...
export default function ResumeBulletRewriter() {
  ...
  return (
    <div className="min-h-screen bg-neutral-50 flex items-center justify-center p-4 font-sans">
      <div className="bg-white/80 backdrop-blur shadow-xl rounded-2xl p-6 sm:p-8 w-full max-w-6xl border border-neutral-200 flex flex-col gap-6">
        <div className="text-center flex flex-col gap-1">
          <h1 className="text-3xl font-bold tracking-tight text-neutral-900">Resume Bullet Rewriter</h1>
          <p className="text-neutral-500 text-base">Turn rough notes into polished resume bullets with Groq-powered rewriting.</p>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1.1fr_0.9fr] gap-4">
          <div className="rounded-2xl border border-neutral-200 bg-white p-4 flex flex-col gap-3">
            <div className="flex items-center justify-between gap-2">
              <h2 className="text-base font-semibold text-neutral-900">Rough notes</h2>
              <span className="text-xs text-neutral-500">One idea per line</span>
            </div>
            <textarea
              value={input}
          ...
          <div className="rounded-2xl border border-neutral-200 bg-white p-4 flex flex-col gap-4">
            <h2 className="text-base font-semibold text-neutral-900">Rewrite controls</h2>
            ...
            <button
              ...
              className={`w-full border border-neutral-900 text-neutral-900 py-2.5 rounded-lg font-semibold hover:bg-neutral-900 hover:text-white transition focus:outline-none focus:ring-2 focus:ring-neutral-900 ${!inputLines.length || loading ? "opacity-50 cursor-not-allowed" : ""}`}
            >
            ...
            <button
              ...
              className={`w-full border border-neutral-300 text-neutral-700 py-2.5 rounded-lg font-semibold hover:bg-neutral-100 transition focus:outline-none focus:ring-2 focus:ring-neutral-900 ${!bullets.length ? "opacity-50 cursor-not-allowed" : ""}`}
            >
              Copy bullets
            </button>
            ...
            <p className="text-xs text-neutral-500">Best for resume lines, project summaries, internship notes, and role descriptions.</p>
          </div>
        </div>

        {message ? <p className="text-sm text-neutral-500">{message}</p> : null}

        <div className="rb-results-shell rounded-2xl border border-neutral-200 bg-neutral-50 p-4">
          <div className="flex items-center justify-between gap-2 mb-3">
            <h2 className="text-base font-semibold text-neutral-900">Rewritten bullets</h2>
            <span className="text-xs text-neutral-500">{bullets.length ? `${bullets.length} bullet(s)` : "Generate to view"}</span>
          </div>

          {!bullets.length ? (
            <p className="text-sm text-neutral-500">Add rough notes and click “Rewrite bullets” to get polished resume-ready lines.</p>
          ) : (
            <div className="grid grid-cols-1 gap-3">
              {bullets.map((bullet, index) => (
                <div key={`${bullet}-${index}`} className="rb-result-card rounded-xl border border-neutral-200 bg-white p-4 flex items-start gap-3">
