...
const INITIAL_SUBJECTS = [
  { id: "sub-1", name: "Data Structures & Algorithms", attended: 28, total: 35, target: 75, medical: 0 },
  { id: "sub-2", name: "Database Management Systems", attended: 20, total: 32, target: 75, medical: 1 },
  { id: "sub-3", name: "Computer Networks Lab", attended: 14, total: 16, target: 80, medical: 0 },
];
...
export default function AttendanceCalculator() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("attendance_calc_data_v2");
      if (saved) {
  ...
  useEffect(() => {
    try {
      ...
      localStorage.setItem("attendance_calc_data_v2", JSON.stringify(dataToSave));
    } catch (e) {
  ...
  const calculations = useMemo(() => {
    ...
    // Medical leave credits
    const medicalClassesCredit = (medicalLeaveDays || 0) * classesPerDay;
    const effectiveTargetClasses = Math.max(
      0,
      Math.ceil(totalClassesPossible * (attendancePercentage / 100)) - medicalClassesCredit
    );
    ...
    const classesFor75 = Math.max(0, Math.ceil(totalClassesPossible * 0.75) - medicalClassesCredit);
    const classesFor80 = Math.max(0, Math.ceil(totalClassesPossible * 0.80) - medicalClassesCredit);
    const classesFor90 = Math.max(0, Math.ceil(totalClassesPossible * 0.90) - medicalClassesCredit);
    ...
    return {
      ...
      classesCanMiss,
      daysCanMiss,
      medicalClassesCredit,
      attendance: totalClassesPossible > 0
        ? (((totalClassesPossible - classesCanMiss) / totalClassesPossible) * 100).toFixed(1)
        : "0.0",
      classesFor75,
      classesFor80,
      classesFor90,
      ...
      availableClassesAfterVacation: Math.max(0, totalClassesPossible - vacationDaysWanted * classesPerDay),
      classesNeededFor75AfterVacation: Math.max(0, Math.ceil((totalClassesPossible - vacationDaysWanted * classesPerDay) * 0.75)),
      classesNeededFor80AfterVacation: Math.max(0, Math.ceil((totalClassesPossible - vacationDaysWanted * classesPerDay) * 0.80)),
      classesNeededFor90AfterVacation: Math.max(0, Math.ceil((totalClassesPossible - vacationDaysWanted * classesPerDay) * 0.90)),
  ...
  const liveMetrics = useMemo(() => {
    ...
    if (isBelowTarget) {
      // Formula for classes needed to reach target:
      // (effectiveAttended + X) / (total + X) >= targetPct / 100
      // 100 * effectiveAttended + 100 * X >= targetPct * total + targetPct * X
      // X * (100 - targetPct) >= targetPct * total - 100 * effectiveAttended
      const numerator = targetPct * total - 100 * effectiveAttended;
      const denominator = 100 - targetPct;
      classesToAttendNeeded = Math.ceil(numerator / denominator);
    } else {
      // Formula for classes safe to bunk:
      // effectiveAttended / (total + Y) >= targetPct / 100
      // 100 * effectiveAttended >= targetPct * total + targetPct * Y
      // Y * targetPct <= 100 * effectiveAttended - targetPct * total
      const numerator = 100 * effectiveAttended - targetPct * total;
      classesCanBunkAllowed = Math.floor(numerator / targetPct);
    }

    return {
      currentPct: currentPct.toFixed(1),
      isBelowTarget,
      classesToAttendNeeded: Math.max(0, classesToAttendNeeded),
      classesCanBunkAllowed: Math.max(0, classesCanBunkAllowed),
      effectiveAttended,
      total,
      targetPct,
    };
  ...
  // Subject-Wise Aggregates & Helper
  const getSubjectStatus = (subject) => {
    ...
    if (isBelow) {
      const num = target * total - 100 * effective;
      const den = 100 - target;
      consecutiveNeeded = Math.max(0, Math.ceil(num / den));
    } else {
      const num = 100 * effective - target * total;
      safeToBunk = Math.max(0, Math.floor(num / target));
    }

    let status = "green";
    if (isBelow) {
      status = "red";
    } else if (safeToBunk <= 2) {
      status = "amber";
    }
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-6 flex flex-col items-center text-center">
          ...
          <p className="text-slate-600 max-w-2xl text-sm sm:text-base">
            Plan your college semester attendance strategically. Predict bunk buffers, catch-up requirements, and track custom subjects in real-time.
          </p>
        </div>

        {/* Segmented Liquid Tab Switcher (Matching Fake-Data-Generator Style) */}
        <div className="relative bg-slate-200/60 p-1.5 rounded-2xl border border-slate-200/50 backdrop-blur-md flex items-center mb-8 shadow-inner">
          ...
          >
            <svg
              ...
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={activeTab === "semester" ? 2.5 : 2}
              stroke="currentColor"
            ...
            <span className="hidden sm:inline">Semester Planner</span>
          ...
          >
            <svg
              ...
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={activeTab === "live" ? 2.5 : 2}
              stroke="currentColor"
              className="w-4 h-4 transition-transform duration-300"
            >
              <path
            ...
            <span className="hidden sm:inline">Catch-Up & Bunk Predictor</span>
          ...
          >
            <svg
              ...
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={activeTab === "subjects" ? 2.5 : 2}
              stroke="currentColor"
              className="w-4 h-4 transition-transform duration-300"
            >
              <path
            ...
            <span className="hidden sm:inline">Subject-Wise Manager</span>
        ...
        <div className="mb-6 bg-white border border-slate-200 rounded-2xl p-4 flex flex-wrap items-center justify-between gap-3 shadow-sm">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition ${
                attendancePercentage === 85 && liveTarget === 85
                  ? "bg-orange-600 text-white shadow-sm"
                  : "bg-slate-100 text-slate-700 hover:bg-orange-50 hover:text-orange-600"
              }`}
            >
            ...
            <button
              onClick={() => {
                setAttendancePercentage(67);
                setLiveTarget(67);
              }}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition ${
                attendancePercentage === 67 && liveTarget === 67
                  ? "bg-orange-600 text-white shadow-sm"
                  : "bg-slate-100 text-slate-700 hover:bg-orange-50 hover:text-orange-600"
              }`}
            >
              67% (Relaxed DU / Central)
            </button>
          </div>
        </div>
        ...
        {activeTab === "semester" && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-1 space-y-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full border border-slate-300 rounded-lg p-2 bg-slate-50 text-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                    />
                  </div>
                ...
                <div className="space-y-4">
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span className="text-2xl font-black bg-gradient-to-r from-orange-600 to-amber-600 text-white px-4 py-1.5 rounded-lg shadow-sm">
                        {attendancePercentage}%
                      </span>
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 pt-2 border-t border-orange-200">
                    <button
                      ...
                      className={`py-2 px-3 rounded-lg font-semibold text-xs transition ${
              ...
              {/* Medical Leave / OD Exemption Input */}
              <div className="bg-white rounded-2xl shadow-md border border-slate-200 p-6">
                <div className="flex items-center justify-between mb-3">
                ...
                <p className="text-xs text-slate-500 mb-3">
                  Medical certificates or Official Duty (OD) credits allowed by college.
                </p>
                <input
                  type="number"
                  min="0"
                  ...
                  onChange={(e) => setMedicalLeaveDays(Math.max(0, parseInt(e.target.value) || 0))}
                  className="w-full border border-slate-300 rounded-lg p-2 bg-slate-50 text-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                />
                <p className="text-xs text-slate-500 mt-1">
                  Adds +{medicalLeaveDays * classesPerDay} lecture credits to target
                </p>
              </div>
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white rounded-2xl shadow-md border border-slate-200 p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="font-semibold text-slate-900 text-sm">Indian Festivals</h3>
                      <button
                  ...
                  <div className="pt-4 border-t border-slate-200">
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="font-semibold text-slate-900 text-sm">General Holidays</h3>
                      <button
                ...
                <div className="space-y-4">
                  ...
                  >
                    <div className="text-center">
                      <p
                        className={`text-base font-bold mb-2 px-3 py-1 rounded-full inline-block ${
                          statusColor === "green"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="text-xs text-slate-500 mt-2">
                          Equivalent to {calculations.classesCanMiss} lectures
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500 mt-2">
                          ({calculations.attendance}% max possible)
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className="w-full bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white font-semibold py-3 px-4 rounded-xl flex items-center justify-between transition-all shadow-md hover:shadow-lg text-sm"
                  >
                    ...
                    <div className="bg-white rounded-2xl border border-slate-200 p-6 animate-in fade-in slide-in-from-top-2 duration-300 space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <input
                            type="text"
                            placeholder="e.g. College Fest / Trip"
                            value={vacationMonth}
                      ...
                      <div className="pt-2">
                        {calculations.canTakeFullVacation ? (
                          <div className="bg-emerald-50 rounded-xl p-4 border border-emerald-200 text-xs">
                            <p className="text-emerald-900 font-bold mb-2 text-sm">
                              ✅ Vacation Approved ({vacationDaysWanted} Days)
                            </p>
                            <p className="text-emerald-800">
                              You will miss {calculations.classesLostToVacation} lectures. Your attendance will remain safely above target!
                            </p>
                          </div>
                        ) : (
                          <div className="bg-rose-50 rounded-xl p-4 border border-rose-200 text-xs">
                            <p className="text-rose-900 font-bold mb-1 text-sm">
                              ⚠️ Warning: Exceeds Safe Limit
                            </p>
                            <p className="text-rose-800">
                              You can only skip {calculations.daysCanMiss} days. Taking {vacationDaysWanted} days will push attendance below {attendancePercentage}%.
                            </p>
                          </div>
                        )}
                      </div>
        ...
        {activeTab === "live" && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-1 space-y-6">
              <div className="bg-white rounded-2xl shadow-md border border-slate-200 p-6 space-y-5">
                <h2 className="text-base font-semibold text-slate-900 flex items-center gap-2">
                  <span>Current Attendance Status</span>
                </h2>
                ...
                <div>
                  ...
                  <input
                    ...
                    max={liveTotal}
                    value={liveAttended}
                    onChange={(e) => setLiveAttended(Math.max(0, parseInt(e.target.value) || 0))}
                    className="w-full border border-slate-300 rounded-lg p-2.5 bg-slate-50 text-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-500 font-bold text-base"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    min="1"
                    value={liveTotal}
                    onChange={(e) => setLiveTotal(Math.max(1, parseInt(e.target.value) || 1))}
                    className="w-full border border-slate-300 rounded-lg p-2.5 bg-slate-50 text-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-500 font-bold text-base"
                  />
                </div>
                ...
                <div>
                  <label className="text-xs font-semibold text-slate-900 block mb-1">
                    Medical / Duty Leave (OD) Classes
                  </label>
                  <input
                    type="number"
                    min="0"
                    value={liveMedicalCount}
                    onChange={(e) => setLiveMedicalCount(Math.max(0, parseInt(e.target.value) || 0))}
                    className="w-full border border-slate-300 rounded-lg p-2 bg-slate-50 text-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
                  />
                ...
                <div className="pt-3 border-t border-slate-200">
                  ...
                  <input
                    ...
                    max="95"
                    value={liveTarget}
                    onChange={(e) => setLiveTarget(parseInt(e.target.value))}
                    className="premium-slider w-full"
                  />
                </div>
              </div>
            ...
            {/* Right Column: Live Analysis & Action Plan */}
            <div className="lg:col-span-2 space-y-6">
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div>
                    ...
                    <div className="flex items-baseline gap-3">
                      <span className="text-5xl font-black text-slate-900">
                        {liveMetrics.currentPct}%
                      </span>
                      <span className="text-xs text-slate-500">
                        ({liveMetrics.effectiveAttended} / {liveMetrics.total} classes)
                      </span>
                    </div>
                  </div>
                  ...
                  <div>
                    {liveMetrics.isBelowTarget ? (
                      <span className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-rose-100 text-rose-800 font-bold text-sm">
                        ⚠️ Below Target ({liveMetrics.targetPct}%)
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-emerald-100 text-emerald-800 font-bold text-sm">
                        🎉 Safe Zone (≥ {liveMetrics.targetPct}%)
                      </span>
                    )}
                  </div>
                </div>
              ...
              {liveMetrics.isBelowTarget ? (
                <div className="bg-gradient-to-br from-rose-50 to-orange-50 rounded-2xl border-2 border-rose-200 p-6 space-y-4 shadow-sm">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-rose-500 text-white flex items-center justify-center font-bold text-xl">
                      🚨
                    </div>
                    <div>
                  ...
                  <div className="bg-white rounded-xl p-5 border border-rose-200 text-center">
                    ...
                    <p className="text-5xl font-black text-rose-600 my-1">
                      {liveMetrics.classesToAttendNeeded}
                    </p>
                    <p className="text-xs text-slate-600 font-medium">
                      upcoming lectures without missing a single class!
                    </p>
                  </div>

                  <div className="text-xs text-slate-600 bg-white/70 rounded-lg p-3 border border-rose-100">
                    💡 <strong>Calculation:</strong> Attending {liveMetrics.classesToAttendNeeded} consecutive lectures will bring your total to{" "}
                    <strong>{liveMetrics.effectiveAttended + liveMetrics.classesToAttendNeeded} / {liveMetrics.total + liveMetrics.classesToAttendNeeded}</strong> classes, achieving exact target of <strong>{liveMetrics.targetPct}%</strong>.
                  </div>
                </div>
              ) : (
                <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl border-2 border-emerald-200 p-6 space-y-4 shadow-sm">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-emerald-500 text-white flex items-center justify-center font-bold text-xl">
                      😎
                    </div>
                    <div>
                  ...
                  <div className="bg-white rounded-xl p-5 border border-emerald-200 text-center">
                    ...
                    <p className="text-5xl font-black text-emerald-600 my-1">
                      {liveMetrics.classesCanBunkAllowed}
                    </p>
                    <p className="text-xs text-slate-600 font-medium">
                      consecutive upcoming lectures and still stay above {liveMetrics.targetPct}%!
                    </p>
                  </div>

                  <div className="text-xs text-slate-600 bg-white/70 rounded-lg p-3 border border-emerald-100">
                    💡 <strong>Calculation:</strong> If you bunk {liveMetrics.classesCanBunkAllowed} upcoming lectures, your attendance will be{" "}
                    <strong>{liveMetrics.effectiveAttended} / {liveMetrics.total + liveMetrics.classesCanBunkAllowed}</strong> ={" "}
                    <strong>{(((liveMetrics.effectiveAttended) / (liveMetrics.total + liveMetrics.classesCanBunkAllowed)) * 100).toFixed(1)}%</strong>.
                  </div>
                </div>
              )}
            </div>
        ...
        {activeTab === "subjects" && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-6 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm">
              <div>
                <h2 className="text-xl font-bold text-slate-900">Custom Subjects Tracker</h2>
                <p className="text-xs text-slate-500">
                  Track individual subjects, practical labs, and lectures separately with custom targets.
                </p>
              </div>

              <button
                onClick={() => setShowAddSubjectForm(!showAddSubjectForm)}
                className="bg-orange-600 hover:bg-orange-700 text-white font-semibold text-xs px-4 py-2.5 rounded-xl transition flex items-center gap-2 shadow-sm"
              >
            ...
            {showAddSubjectForm && (
              <form
                ...
                className="bg-orange-50/70 border-2 border-orange-200 rounded-2xl p-6 space-y-4 animate-in fade-in slide-in-from-top-2 duration-300"
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                  <div className="lg:col-span-2">
                    ...
                    <input
                      ...
                      required
                      className="w-full border border-slate-300 rounded-lg p-2 bg-white text-slate-900 text-xs focus:ring-2 focus:ring-orange-500"
                    />
                  </div>
                ...
                <div className="flex justify-end gap-2 pt-2">
                  <button
                    ...
                    className="px-4 py-2 rounded-lg text-xs font-semibold border border-slate-300 bg-white hover:bg-slate-100"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="px-4 py-2 rounded-lg text-xs font-semibold bg-orange-600 text-white hover:bg-orange-700"
                  >
                    Save Subject
                  </button>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {subjects.map((sub) => {
                ...
                return (
                  <div
                    key={sub.id}
                    className={`rounded-2xl border-2 p-5 bg-white transition-all shadow-sm ${
                      subMeta.status === "green"
                        ? "border-emerald-200 hover:border-emerald-400"
                        : subMeta.status === "amber"
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        ...
                        <p className="text-xs text-slate-500">
                          Target: <span className="font-bold">{sub.target}%</span>
                        </p>
                      </div>
                    ...
                    <div className="grid grid-cols-3 gap-2 bg-slate-50 p-2.5 rounded-xl border border-slate-200 text-xs mb-3">
                      <div>
                        ...
                        <input
                          ...
                          value={sub.attended}
                          onChange={(e) =>
                            handleUpdateSubject(sub.id, "attended", Math.max(0, parseInt(e.target.value) || 0))
                          }
                          className="w-full border border-slate-300 rounded p-1 bg-white font-semibold"
                        />
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          value={sub.total}
                          onChange={(e) =>
                            handleUpdateSubject(sub.id, "total", Math.max(1, parseInt(e.target.value) || 1))
                          }
                          className="w-full border border-slate-300 rounded p-1 bg-white font-semibold"
                        />
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          value={sub.medical || 0}
                          onChange={(e) =>
                            handleUpdateSubject(sub.id, "medical", Math.max(0, parseInt(e.target.value) || 0))
                          }
                          className="w-full border border-slate-300 rounded p-1 bg-white font-semibold"
                        />
                      </div>
        ...
        <div className="mt-8 bg-orange-50 border border-orange-200 rounded-2xl p-6">
          <p className="text-xs sm:text-sm text-orange-900">
            <strong>Tip:</strong> Your attendance data and custom subjects are automatically saved locally on your browser. Easily switch between Semester Planner, Catch-Up Predictor, and Subject-Wise Manager to keep your attendance stress-free!
          </p>
        </div>
      </div>
