...
export default function Home() {
  ...
  // User preferences (Pinned & Recent Tools)
  const { pinnedIds, recentIds, togglePin, clearRecents, isPinned } = useToolPreferences();
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans">
            ...
            {featuredTools.map((tool) => {
              ...
              return (
                <a
                  ...
                  className={`group block p-6 rounded-xl border transition ${
                    isTimeCapsule 
                  ...
                  <div className="flex items-start justify-between mb-3 relative z-10">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        ...
                        title={isPinned(tool.id) ? "Unpin tool" : "Pin tool for quick access"}
      ...
      <section id="find-tools" ref={findToolsRef} className="bg-slate-50 py-12 sm:py-16">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          {/* Quick Access (Pinned & Recent Tools) - only visible if user has preferences */}
          {(pinnedToolsList.length > 0 || recentToolsList.length > 0) && (
            <div className="mb-10 p-5 rounded-2xl border border-slate-200/90 bg-white shadow-xs">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full bg-orange-500" />
                  <h3 className="font-bold text-slate-900 text-sm sm:text-base">Your Quick Access</h3>
                </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {pinnedToolsList.map((tool) => (
                      <a
                        ...
                        href={tool.href}
                        onClick={() => recordRecentTool(tool.id)}
                        className="group inline-flex items-center gap-2 px-3 py-1.5 rounded-xl border border-amber-200 bg-amber-50/40 hover:border-amber-400 hover:bg-amber-50/80 text-xs font-medium text-slate-800 transition shadow-2xs"
                      >
                        <span>{tool.name}</span>
                        <button
                          type="button"
                  ...
                  <div className="flex flex-wrap gap-2">
                    {recentToolsList.map((tool) => (
                      <a
                        ...
                        href={tool.href}
                        onClick={() => recordRecentTool(tool.id)}
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-slate-200 bg-white hover:border-orange-300 hover:bg-orange-50/30 text-xs text-slate-700 transition shadow-2xs"
                      >
                      ...
                      <div className="flex items-start justify-between mb-2 relative z-10">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <button
                            ...
                            title={pinned ? "Unpin tool" : "Pin tool for quick access"}
                            className={`p-1 rounded-md transition cursor-pointer ${
                              pinned
                                ? "text-amber-500 hover:text-amber-600 bg-amber-50"
                                : "text-slate-300 hover:text-amber-500 hover:bg-slate-100 opacity-60 group-hover:opacity-100"
